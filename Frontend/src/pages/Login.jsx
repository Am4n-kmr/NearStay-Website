  ...
  return (
    <div className="min-h-screen flex">
      ...
      <div
        ...
        style={{ background: "linear-gradient(145deg, #0f0c29 0%, #1a1060 50%, #24243e 100%)" }}
      >
        {/* Glow orbs */}
        <div className="absolute top-[-15%] left-[-10%] w-80 h-80 rounded-full opacity-25 blur-3xl pointer-events-none"
        ...
        <div className="relative flex items-center gap-2.5">
          ...
          <span className="font-bold text-white text-xl tracking-tight">NearStay</span>
        </div>
        ...
        <div className="relative">
          ...
          <p className="text-white/60 leading-relaxed text-sm mb-10 max-w-xs">
            Verified PGs, hostels, and shared rooms near your college — direct from owners, no middlemen.
          </p>

          <div className="space-y-3 mb-10">
            {["Verified property listings", "Direct contact with owners", "Secure Razorpay payments", "Student-reviewed ratings"].map(f => (
          ...
          <div className="bg-white/[0.08] backdrop-blur border border-white/15 rounded-2xl p-5">
            ...
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 rounded-full bg-violet-500 flex items-center justify-center text-white font-bold text-xs shrink-0">P</div>
              <div>
                <div className="text-white text-xs font-semibold">{TESTIMONIAL.name}</div>
                <div className="text-white/50 text-xs">{TESTIMONIAL.college}</div>
              </div>
            </div>
          </div>
        </div>

        <p className="relative text-white/30 text-xs">&copy; {new Date().getFullYear()} NearStay</p>
      </div>
      ...
      <div className="flex-1 flex flex-col justify-center px-6 py-12 lg:px-16 bg-background">
        ...
        <div className="max-w-sm w-full mx-auto">
          ...
          <p className="text-muted-foreground text-sm mb-8">Sign in to continue to NearStay</p>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <button type="submit"
              className="w-full h-11 bg-primary text-white font-semibold text-sm rounded-lg transition-all active:scale-[0.98] disabled:opacity-60"
              disabled={isLoading}>

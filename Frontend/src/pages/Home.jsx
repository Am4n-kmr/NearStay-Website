...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <nav className="sticky top-0 z-50 border-b border-border bg-background/80 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-4 h-14 sm:h-16 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <button onClick={() => navigate("/register")} className="text-sm font-semibold px-4 py-1.5 rounded-lg bg-primary text-white hover:bg-primary/90 transition-colors">
              Get started
            </button>
          </div>
        </div>
      ...
      >
        {/* Glow orbs */}
        <div className="absolute top-[-10%] left-[-5%] w-[40vw] h-[40vw] max-w-lg rounded-full opacity-20 blur-3xl pointer-events-none"
          style={{ background: "radial-gradient(circle, #6366f1, transparent 70%)" }} />
        <div className="absolute bottom-[-10%] right-[-5%] w-[35vw] h-[35vw] max-w-md rounded-full opacity-15 blur-3xl pointer-events-none"
        ...
        <div className="relative max-w-7xl mx-auto px-4 pt-14 pb-16 sm:pt-20 sm:pb-24 lg:pt-24 lg:pb-28 grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          {/* Left content */}
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-white/20 bg-white/10 backdrop-blur text-white/80 text-xs font-medium mb-6">
            ...
            {/* Search bar */}
            <div className="flex gap-2 bg-white/10 backdrop-blur border border-white/20 rounded-xl p-1.5 max-w-lg">
              ...
              <button
                ...
                className="flex items-center gap-2 bg-primary hover:bg-primary/90 text-white text-sm font-semibold px-4 py-2.5 rounded-lg transition-all active:scale-95 shrink-0"
              >
            ...
            <div className="flex flex-wrap gap-2 mt-4">
              ...
              {["Mumbai", "Delhi", "Bangalore", "Pune"].map(city => (
                <button
                  ...
                  className="text-xs px-3 py-1 rounded-full border border-white/20 text-white/70 hover:bg-white/10 hover:text-white transition-all"
                >
                  {city}
                </button>
              ))}
          ...
          <div className="hidden lg:flex flex-col h-72">
      ...
      <section className="max-w-7xl mx-auto px-4 py-10 sm:py-12">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
          {TRUST_BADGES.map(({ icon: Icon, label, desc }) => (
            <div key={label} className="flex flex-col items-center text-center p-4 sm:p-5 rounded-xl bg-card border border-border hover:border-primary/40 transition-all duration-200 hover:shadow-sm">
      ...
      <section className="max-w-7xl mx-auto px-4 pb-12 sm:pb-14">
        <div className="flex items-center justify-between mb-5 sm:mb-6">
          ...
          <button onClick={() => navigate("/search")} className="flex items-center gap-1 text-primary text-sm hover:underline transition-colors">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
            {POPULAR_CITIES.map(city => (
              <button
                key={city}
                onClick={() => handleCitySearch(city)}
                className="relative overflow-hidden rounded-xl border border-border bg-card hover:border-primary/50 hover:shadow-sm active:scale-[0.97] transition-all duration-150 p-4 text-left group"
              >
      ...
      <section className="max-w-7xl mx-auto px-4 py-12 sm:py-16">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
          {HOW_IT_WORKS.map(({ step, title, desc }, i) => (
            <div key={step} className="text-center flex flex-col items-center sm:block">
              <div className="flex items-center gap-4 sm:flex-col sm:items-center sm:gap-0">
                <div className="w-12 h-12 rounded-full bg-primary text-white font-bold text-lg flex items-center justify-center sm:mx-auto sm:mb-4 shrink-0">
                  {step}
                </div>
      ...
      >
        <div className="absolute top-[-20%] right-[-5%] w-80 h-80 rounded-full opacity-20 blur-3xl pointer-events-none"
          style={{ background: "radial-gradient(circle, #6366f1, transparent 70%)" }} />
        <div className="relative max-w-7xl mx-auto px-4 text-center">
          <h2 className="text-xl sm:text-2xl md:text-3xl font-bold mb-3">Own a property? List it on NearStay</h2>
          <p className="text-white/70 mb-6 max-w-lg mx-auto text-sm sm:text-base">
            Reach thousands of verified students looking for accommodation. Get bookings within days.
          </p>
          <button
            onClick={() => navigate("/register")}
            className="bg-white text-indigo-700 hover:bg-white/90 active:scale-[0.98] transition-all font-semibold px-6 py-3 rounded-lg text-sm"
          >

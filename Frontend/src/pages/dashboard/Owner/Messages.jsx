...
export default function StudentMessages() {
  ...
  const otherParticipant = selectedChat?.participants.find(p => p.id !== String(user?.id));
  ...
  const handleSend = async () => {
    ...
    setMsgText("");
    try {
      await sendMessage.mutateAsync({ chatId: selectedChatId, data: { content: text } });
        ...
        <div className={cn("w-full md:w-72 border-r border-border flex flex-col", selectedChatId ? "hidden md:flex" : "flex")}>
          ...
          <div className="flex-1 overflow-y-auto">
            {chatsLoading ? Array.from({ length: 4 }).map((_, i) => (
              ...
              const other = chat.participants.find(p => p.id !== String(user?.id));
              return (
                <button key={chat.id} onClick={() => setSelectedChatId(chat.id)} className={cn("w-full p-4 flex gap-3 items-start hover:bg-muted transition-colors text-left", selectedChatId === chat.id ? "bg-muted" : "")}>
